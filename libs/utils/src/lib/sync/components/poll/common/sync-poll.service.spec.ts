...
describe('SyncPoll', () => {
  ...
  describe('calculateUserScore', () => {
    ...
    it('handles simple case when the first user is right', () => {
      const v = calculateUserScore(testConfig, presenterData, {a: userData.dogIsRight});
      expect(v).toEqual({dog: 100, cat: 0, pikachu: 0, mouse: 0});
    });

    it('handles simple case when the second user is right immideately', () => {
      const v = calculateUserScore(testConfig, presenterData, {b: userData.catIsRightImmediately});
      expect(v).toEqual({dog: 0, cat: 100, pikachu: 0});
    });

    it('two users gave correct answer immediately', () => {
      const v = calculateUserScore(testConfig, presenterData, {b: userData.catAndDogAreRightImmediately});
      expect(v).toEqual({dog: 100, cat: 100, pikachu: 0});
    });

    it('second user gave correct answer at the same time', () => {
      const v = calculateUserScore(testConfig, presenterData, {b: userData.catAndDogAreRightAtTheSameTime});
      expect(v).toEqual({dog: 100, cat: 100, pikachu: 0});
    });

    it('2 users gave correct answer, but cat was faster', () => {
      const v = calculateUserScore(testConfig, presenterData, {b: userData.everyoneIsRight});
    ...
    it('Ignores answers that are outside of the range', () => {
      const v = calculateUserScore(testConfig, presenterData, {b: userData.outsideOfRange});
