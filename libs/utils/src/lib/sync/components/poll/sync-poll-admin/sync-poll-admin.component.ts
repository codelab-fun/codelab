...
export class SyncPollAdminComponent implements OnInit, OnChanges {
  ...
  pollEnabled: boolean;
  poll!: SyncPoll;
  private readonly timing = this.syncDataService.getPresenterObject('poll-timing');
  ...
  ngOnChanges(changes) {
    if (changes.config && changes.config.currentValue) {
      this.timing.object(changes.config.currentValue.key).set(database.ServerValue.TIMESTAMP);
    }
  }
