...
import { distinctUntilChanged, filter, first, map, mergeMapTo, switchMap, takeUntil, tap } from 'rxjs/operators';
...
export class SyncUserValueDirective<T> implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    const $key = this.loginService.uid$.pipe(map(uid => `user-data/${uid}/${this.syncUserValue}`));
