...
describe('Component Tree', () => {
  ...
  describe('Make sure things work', () => {
    it(`Thumbs.Component.ts: Add the '@Component' decorator and set its selector to be 'my-thumbs'.`, () => {
      ...
      chai.expect(
        metadata,
        `ThumbsComponent doesn't have a @Component() annotation`
      ).is.not.undefined;
      chai
        .expect(
          metadata.selector,
          `ThumbsComponent's selector has to be 'my-thumbs'.`
        )
        .equals('my-thumbs');
    });
    ...
    it(`Thumbs.Component.ts: Set the templateUrl to load appropriate html file.`, () => {
      ...
      chai
        .expect(
          ...
          `ThumbsComponent's templateUrl should be set to './thumbs.html'`
        )
        .equals('./thumbs.html');
    });
    ...
    it(`Thumbs.Component.ts: Add an 'onThumbs' property and set the value to a new EventEmitter. Decorate with @Output()`, () => {
      const metadata = ThumbsComponent['__annotations__'][0];
      chai.expect(metadata, `ThumbsComponent doesn't have any @Outputs()'s`).is
        .not.undefined;
      chai
        .expect(
          Object.keys(metadata).length,
          `ThumbsComponent doesn't have any @Outputs()'s`
        )
        .equals(1);
      chai.expect(
        metadata.onThumbs,
        `ThumbsComponent's @Outputs()' should be called onThumbs.`
      ).is.not.undefined;
    });
  });
  ...
  describe('Make sure things are displayed properly', () => {
    ...
    it(`thumbs.html: Make the 'thumbs-up' button emit the onThumbs event with the correct thumbs ENUM value.`, () => {
      ...
      chai.expect(thumbs, `OnThumbs was called without pressing the button`).to
        .be.not.ok;
      fixture.nativeElement.querySelector('.thumbs-up').click();
      chai
        .expect(
          thumbs,
          `OnThumbs was not called when pressing the button with the 'thumbs-up' class.`
        )
        .to.equal(Thumbs.UP);
    });
    ...
    it(`thumbs.html: Make the 'thumbs-down' button emit the onThumbs event with the correct thumbs ENUM value.`, () => {
      ...
      chai
        .expect(
          thumbs,
          `OnThumbs was not called when pressing the button with the 'thumbs-down' class.`
        )
        .to.equal(Thumbs.DOWN);
    });
  });
});
