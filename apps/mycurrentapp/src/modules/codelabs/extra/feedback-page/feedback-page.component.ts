...
Author: ${message.name}
Slide: [Local](http://localhost:4200${message.href}),[Public](https://angular-presentation.firebaseapp.com${
      message.href
    })`;
  }
    ...
    this.ghService
      ...
      .subscribe(response => {
        if (response.ok) {
          ...
          this.database.object(`feedback/${message.key}`).update({ url: responseData.html_url });
          this.ghService
            .closeIssue({ state: 'closed' }, responseData.number, this.githubAuth.credential.accessToken)
            .subscribe(res => {
              if (res.ok) {
                this.isDone(message);
              }
