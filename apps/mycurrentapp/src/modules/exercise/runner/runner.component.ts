  ...
  return new Promise((resolve, reject) => {
    ...
    iframe.contentWindow.onload = () => {
      ...
      const displayError = (error, info) => {
         ...
         */
        const escaped = (document.createElement('a').appendChild(document.createTextNode(error)).parentNode as any)
          .innerHTML;
        setHtml(`
        ...
        runMultipleFiles: (files: Array<FileConfig>) => {
            ...
            return {
              ...
              execute: function() {
                ...
                files.forEach(file => {
                  ...
                  exports(
                    file.path.replace(/[\/\.-]/gi, '_') + '_AST',
                    ts.createSourceFile(file.path, file.code, ts.ScriptTarget.ES5)
                  );
                });
              }
            };
          ...
          if (diagnostics.length) {
            ...
            const diagnosticsHtml = diagnostics
              .map(
                diagnostic =>
                  `<li>Error in file <b>${diagnostic.file.fileName}</b>:
            ` +
...
export class RunnerComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  constructor(public loopProtectionService: LoopProtectionService, public scriptLoaderService: ScriptLoaderService) {
  ...
  fullUrl() {
    return this.fakeUrl || (this.urlBase + this.url).replace(this.hiddenUrlPart, '').replace('about:blank', '');
  }
  ...
  runCode(files: Array<FileConfig>, runner: string): void {
    ...
    if (runner === 'Angular') {
      ...
      injectIframe(
        ...
        sandbox.runCss(require('./inner.css'));
        sandbox.setHtml('<div id="modules"></div>');
