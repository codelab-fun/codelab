...
export function gifParser(t: { [key: string]: string; }) {
  ...
  const palette = new BinaryParser()
    .array('palette', {
      ...
      length(data) {
        const paletteSize = data._parent[0].value.find(a => a.name === 'paletteSize').value;
  ...
  const imageDescriptorParser = new BinaryParser()
    ...
    .boolean('isImageInterlacingEnabld', {description: t.isImageInterlacingEnabld})
    .boolean('isLocalPaletteSorted', {description: t.isLocalPaletteSorted})
    .constBits('00', {description: t.reservedBits})
    .bit3('localPaletteSize', {type: 'enums', description: t.localPaletteSize})
    .uInt8('colorDepth')
    .uInt8('blockSize')
    .bit('graphicBlock', {
